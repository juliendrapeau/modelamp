...
class VariableManager:
    ...
    def __init__(self):
        ...
        self.io_map: Dict[int, int] = {}  # Maps qubit index -> current output CNF variable
...
class CircuitToCNFConverter:
    """
    Converts a Qiskit QuantumCircuit into a CNF representation with complex weights
    for use in probabilistic or quantum model counting.
    """
    def __init__(self):
    ...
    def _add_general_unitary(self, unitary: np.ndarray, qubit_indices: List[int]):
        ...
        if unitary.shape != (2 ** k, 2 ** k):
            raise ValueError(f"Expected shape {(2**k, 2**k)} for a {k}-qubit gate, got {unitary.shape}.")
    ...
    def _dispatch_gate(self, instr):
        ...
        if hasattr(instr, "matrix"):
            self._add_general_unitary(instr.matrix, qubit_indices)
        else:
            raise NotImplementedError(f"Unsupported gate type: {instr.operation.name}")
    ...
    def export_dimacs(self, filename: Optional[str] = None) -> str:
        ...
        num_vars = max(abs(lit) for clause in self.clauses for lit in clause)
        ...
        for lit, w in self.weights.items():
            if not isinstance(w, complex):
                raise TypeError(f"Expected complex weight for literal {lit}, got {type(w)}")
            r, i = w.real, w.imag
